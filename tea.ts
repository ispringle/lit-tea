...
export const cmd = <M extends object>(
  model: M,
  ...effects: Effect[]
): UpdateResult => [model, ...effects];

type Constructor<T = LitElement> = new (...args: any[]) => T;

type TeaRootInstance<M, Msg> = {
  model: M;
  dispatch: (msg: Msg) => void;
};

export const TeaRoot =
  <M extends object, Msg>(init: M, update: Update<M, Msg>) =>
  <T extends Constructor>(
    Base: T,
  ): T & (new (...args: any[]) => TeaRootInstance<M, Msg>) => {
    class TeaRootElement extends Base {
      static properties = { _model: { state: true } };
      _model: M = init;
      _controllers: AbortController[] = [];

      dispatch = (msg: Msg) => {
        const result = update(this._model, msg);
        const [newModel, ...effects] = Array.isArray(result)
          ? result
          : [result];
    ...
    return TeaRootElement as any;
  };

type TeaLeafInstance<M, K extends keyof M> = {
  model: M;
  dispatch: (msg: any) => void;
  slice: Pick<M, K>;
  send: (msg: any) => void;
...
export const TeaLeaf = <M extends object, K extends keyof M>(
  ...keys: K[]
): new (...args: any[]) => TeaLeafInstance<M, K> & LitElement => {
